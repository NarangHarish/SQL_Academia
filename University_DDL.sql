...
INSERT INTO Student VALUES ('345678901','WALLY','KEN
DALL','SEATTLE','WA','98123-1141','IS','SR',2.80);
...
select * from faculty
select str_to_date(FacHireDate, '%m/%d/%y') from faculty
-- alter table faculty add column FacHireDate1 date;
-- alter table faculty drop column FacHireDate1;
-- insert into faculty (FacHireDate1) 
-- select str_to_date(FacHireDate, '%m/%d/%y');

alter table offering add constraint CourseFK FOREIGN key (CourseNo)  REFERENCES Course(CourseNo)
on update cascade;
alter table offering add CONSTRAINT facultyFK FOREIGN KEY (FacNo) REFERENCES Faculty (FacNo)
on update cascade;
alter table enrollment add CONSTRAINT OfferingFK FOREIGN KEY (OfferNo) REFERENCES Offering(OfferNo)
on update cascade;
alter table enrollment add CONSTRAINT StudentFK FOREIGN KEY (StdNo) REFERENCES Student(StdNo)
on update cascade;
